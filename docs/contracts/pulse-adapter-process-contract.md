# عقد عملية PulseAdapter — الإصدار 1.1
...
| checkpoint صريح | `SAVE` يكتب state محليًا ويحفظ SHA-256 | لا تقبل الاستعادة ملفًا خارج مجلد حالة adapter. |
...
يلزم أن تكون ملفات الحالة داخل `state_directory` الخاص بالـadapter، وأن تطابق SHA-256 المسجلة قبل `RESTORE`. لا يمثّل digest تشفيرًا أو حماية وصول؛ إنه فحص سلامة ونسبة artifact فقط. عند عدم تمرير `PulseAdapterConfig.state_directory` ينشئ adapter مجلدًا مؤقتًا ويحذفه عند `close()`؛ ولذلك لا تكون artifact الافتراضية دائمة عبر إغلاق adapter.

## اللقطات الكانونية مقابل فروع checkpoint

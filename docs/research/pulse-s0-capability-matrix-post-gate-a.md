...
| checkpoint عبر إغلاق Runtime / جلسة جديدة | **PARTIAL** | `PulseAdapterConfig.state_directory` يسمح بمسار مُعد صراحة؛ [عقد adapter](../contracts/pulse-adapter-process-contract.md). | المسار الافتراضي مؤقت ويحذف عند `close()`؛ لا يوجد اختبار استعادة عبر إعادة تشغيل Runtime. | تعامل مع artifact كـin-session فقط في S0؛ لا تدّع branch replay عبر الجلسات. | **>5 أيام**؛ يحتاج سياسة تخزين، اختبار restart، ومراجعة أمنية. |
| التكرارية العامة | **PARTIAL** | [Gate A، §4.3](../reports/002-pulse-gate-a.md#43-استمرارية-الحفظ-والاستعادة) يثبت تطابق الاستمرارية لمسار محدد؛ [benchmark](../../artifacts/benchmarks/pre_m3_runtime_hardening/pulse_client_operation_summary.json) يعرض خمس عينات محلية. | لا يوجد ادعاء حتمية عامة عبر منصات أو مسارات أو إصدارات. | تثبيت revision والحالة والأوامر وتسجيل evidence لكل تجربة. | **3–5 أيام** لتوسيع harness إلى مصفوفة مسارات/بيئات محددة؛ لا يثبت determinism عامًا. |

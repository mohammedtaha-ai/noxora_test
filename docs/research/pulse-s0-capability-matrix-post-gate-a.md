...
| زمن محاكاة مملوك لـVPE | **SUPPORTED** | [اختبارات Runtime](../../tests/test_runtime.py) و[تكامل Pulse](../../tests/test_pulse_adapter_integration.py) يثبتان `advance_time` عبر adapter. | التقدم `60s` و`120s` حاجب في القياس المحلي. | تنفيذ التشغيل خارج خيط رسم Unity؛ لا يعالج ذلك latency المحرك. | جزء من تصميم M3؛ لا يقدّر قبل عقد Facade. |
...
| إيقاف النزف | **SUPPORTED** | [Gate A، §4.1](../reports/002-pulse-gate-a.md#41-النزف-الداخلي-الطحالي): يصبح المعدل `0.000 mL/s` عند `1261s`. | مسار Gate A تشخيصي؛ ليس فعلًا مكشوفًا كخيار متعلم في S0. | يبقى خارج قاموس تدخلات S0 حتى اعتماد المحتوى. | مراجعة محتوى وعقد؛ غير مقدرة. |
...
| مخرج الأكسجة | **PARTIAL** | `oxygen_saturation` ضمن مفاتيح telemetry المحصورة، وتتحقق اختبارات Pulse من أن القيم منتهية. | لا توجد تجربة Gate A مستقلة تفسر سلوكه أو مراجعته طبيًا. | عرضه كقيمة محرك فقط بعد مراجعة المحتوى؛ أو إخفاؤه في M3. | مراجعة محتوى؛ غير مقدرة. |
...
| checkpoint عبر إغلاق Runtime / جلسة جديدة | **PARTIAL** | `PulseAdapterConfig.state_directory` يسمح بمسار مُعد صراحة؛ [عقد adapter](../contracts/pulse-adapter-process-contract.md). | المسار الافتراضي مؤقت ويحذف عند `close()`؛ لا يوجد اختبار استعادة عبر إعادة تشغيل Runtime. | تعامل مع artifact كـin-session فقط في S0؛ لا تدّع branch replay عبر الجلسات. | مؤجل؛ يحتاج سياسة تخزين واختبار واستعراض أمني. |
| التكرارية العامة | **PARTIAL** | [Gate A، §4.3](../reports/002-pulse-gate-a.md#43-استمرارية-الحفظ-والاستعادة) يثبت تطابق الاستمرارية لمسار محدد؛ [benchmark](../../artifacts/benchmarks/pre_m3_runtime_hardening/pulse_client_operation_summary.json) يعرض خمس عينات محلية. | لا يوجد ادعاء حتمية عامة عبر منصات أو مسارات أو إصدارات. | تثبيت revision والحالة والأوامر وتسجيل evidence لكل تجربة. | منفذ جزئيًا؛ توسيع harness غير مقدر. |
| TXA | **ABSENT** | خارج قاموس S0 واختبارات Gate A. | لا يوجد تحقق محلي أو مراجعة محتوى. | يبقى مؤجلًا خارج S0. | غير مقدرة؛ يلزم نطاق ومراجعة مستقلان. |
| تعديل وعائي / vasopressor | **ABSENT** | خارج قاموس S0 واختبارات Gate A. | لا يوجد تحقق محلي أو مراجعة محتوى. | يبقى مؤجلًا خارج S0. | غير مقدرة؛ يلزم نطاق ومراجعة مستقلان. |

## دلالة المصفوفة على Gate A وM3

يثبت الدليل أن محرك Pulse يمكن تشغيله عبر عقد S0 الضيق، وأن مسارات النزف، التدخلات المحددة، الحفظ والاستعادة متاحة **هندسيًا** على المرجع المثبت. لا ترفع هذه المصفوفة Gate A إلى مصادقة طبية، ولا تغلق Gate 0، ولا تثبت أن واجهة Unity يجب أن تبدأ. وخصوصًا، تبقى حدود التكرارية عبر الجلسات، مراجعة محتوى العميل، وصلاحية العرض التعليمي مسائل منفصلة.

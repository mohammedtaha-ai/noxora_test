...
final class RequestSimulationStart
{
    ...
    public function handle(
        ...
        return DB::transaction(function () use ($actor, $tenantId, $assignmentId, $requestId, $correlationId, $forceOutboxFailure): array {
            // This PostgreSQL transaction-scoped lock serializes concurrent retries of one actor/request key.
            DB::select('SELECT pg_advisory_xact_lock(hashtext(?))', [$actor->id.'|'.$requestId]);

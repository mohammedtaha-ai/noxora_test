...
The integration tests require `PLATFORM_TEST_DATABASE_URL`, defaulting only to the documented local `nexora_control_plane_test` URL. They use real PostgreSQL migrations, transactions, concurrent allocation attempts, lease expiry/reclaim, stale-owner fencing, and the shared contract fixtures. No platform test claims a Docker/broker/VPE/Pulse integration.
